...
export function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { cls: string; label: string }> = {
    MATCHED: { cls: "bg-emerald-100 text-emerald-800", label: "✓ Matched" },
    MISMATCH: { cls: "bg-amber-100 text-amber-800", label: "⚠ Amount mismatch" },
    UNMATCHED: { cls: "bg-rose-100 text-rose-800", label: "✗ Unmatched" },
    PAID: { cls: "bg-emerald-100 text-emerald-800", label: "Paid" },
    PARTIAL: { cls: "bg-amber-100 text-amber-800", label: "Partial" },
    PENDING: { cls: "bg-slate-100 text-slate-700", label: "Pending" },
  };
