...
export function VoiceInvoice({ onCreated }: { onCreated: () => void }) {
  ...
  async function stopRecording() {
    ...
    try {
      ...
      const res = await fetch("/api/billing/invoices/voice", { method: "POST", body: form });
          ...
          {result.invoice ? (
            <div className="flex items-center gap-4 bg-emerald-50 rounded-lg p-3">
              ...
              <div className="text-sm">
                <div className="font-semibold text-emerald-800">Bill made ✓</div>
            ...
            <p className="text-sm text-amber-700 bg-amber-50 rounded-lg p-3">
              Could not catch the amount. Please type it in the form below.
            </p>
          )}
        </div>
      )}
